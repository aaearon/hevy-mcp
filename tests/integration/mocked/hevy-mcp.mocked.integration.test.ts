...
describe("Hevy MCP Server Mocked Integration Tests", () => {
	...
	it("mocks get-workouts through MCP client/server transport", async () => {
		...
		const payload = JSON.parse(result.text) as {
			workouts: Array<{ id: string; title: string; duration: string }>;
		};
	...
	it("mocks get-routines through MCP client/server transport", async () => {
		...
		const payload = JSON.parse(result.text) as {
			routines: Array<{ id: string; title: string; folderId: number }>;
		};

		expect(result.isError).toBeFalsy();
		expect(payload.routines[0]).toMatchObject({
			id: "routine-1",
			title: "Mock Push Day",
			folderId: 10,
		});
	...
	it("mocks get-exercise-templates through MCP transport", async () => {
		...
		expect(result.isError).toBeFalsy();
		expect(payload.exerciseTemplates[0]).toMatchObject({
			id: "template-1",
			title: "Bench Press",
	...
	it("mocks get-routine-folders through MCP transport", async () => {
		...
		const payload = JSON.parse(result.text) as {
			routineFolders: Array<{ id: number; title: string }>;
		};

		expect(result.isError).toBeFalsy();
		expect(payload.routineFolders[0]).toMatchObject({
			id: 10,
			title: "Mock Folder",
		});
	});
	...
	it("mocks a write tool through MCP transport", async () => {
		...
		const payload = JSON.parse(result.text) as {
			routineFolder: { id: number; title: string };
		};

		expect(result.isError).toBeFalsy();
		expect(payload.routineFolder).toMatchObject({
			id: 99,
			title: "Created Folder",
		});
	});
