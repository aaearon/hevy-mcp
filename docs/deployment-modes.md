...
This fork ships no runtime telemetry and performs no update check, so there is
no telemetry opt-out setting and no update-check cache. `HEVY_MCP_DEBUG` is the
only diagnostic, and it never leaves the machine. See the "Telemetry and
network activity" section of `README.md`.
