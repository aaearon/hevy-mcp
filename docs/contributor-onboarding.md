...
| `packages/node/`        | `hevy-mcp`              | Node.js stdio/HTTP/`http+oauth` adapter, process lifecycle                |
...
- **`packages/node`** — stdio transport, process lifecycle (`SIGTERM`/`SIGINT`), the fork-only `http+oauth` transport
