...
| Response contracts     | `packages/core/src/utils/output-schemas.ts`, `formatters.ts`, `response-contracts.ts` | Focused modules own Zod output schemas, raw-to-public formatting, legacy text projection, and response assembly. |
...
1. **Production response contracts are the oracle.** Export reusable,
   ...
   `packages/core/src/utils/output-schemas.ts`. Raw-to-public normalization lives
   in `packages/core/src/utils/formatters.ts`, while legacy text projection and
   response assembly live in `packages/core/src/utils/response-contracts.ts`. Use
   those schemas in mocked MCP, live Vitest, and spawned canary assertions. Tests
