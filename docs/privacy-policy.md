...
- Hosted MCP activity spans may contain the Cloudflare colo that processed the
  request, which is a regional edge proxy rather than exact user geography.
  This fork derives no pseudonymous user identity from the API key.
...
Hosted activity spans containing `span.cloudflare.colo` are retained for 30 days
and are accessible only to repository maintainers and the on-call operator. This
fork emits no `span.user.hash`.
...
their Hevy API key. The locally run software emits no telemetry at all and
contacts no host other than `api.hevyapp.com`; see the repository README for
details.
