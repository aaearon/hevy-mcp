...
describe("telemetry initialization", () => {
	...
	// Fork-specific: Sentry telemetry is opt-in. No DSN is baked in, so nothing
	// is ever reported to a Sentry project unless the user sets SENTRY_DSN.
	it("initializes Sentry with dsn undefined when SENTRY_DSN is unset", async () => {
		vi.resetModules();
		process.env = { ...originalEnv };
