...
export async function startStreamableHttpServer(
	...
	async function resolveRequestSession(
		request: IncomingMessage,
		response: ServerResponse,
	): Promise<RequestSessionResolution | undefined> {
		if (
			!validateHostHeader(
				request,
		...
		if (extensions?.authorize) {
			if (!(await extensions.authorize(request, response))) return undefined;
	...
	async function handleRequest(
		...
		try {
			const resolution = await resolveRequestSession(request, response);
