...
function renderedJson(response: McpToolResponse): unknown {
...
describe("routine tools", () => {
	...
	it("unwraps the wrapped-array routine returned by create and update", async () => {
		...
		const client = createMockHevyClient();
		client.createRoutine.mockResolvedValue({ routine: [created] });
		client.updateRoutine.mockResolvedValue({ routine: [created] });
		const tool = register(client);
	...
	it("unwraps a singular routine wrapper and a bare routine body", async () => {
		...
		const client = createMockHevyClient();
		client.createRoutine.mockResolvedValue({ routine: created });
		client.updateRoutine.mockResolvedValue(created);
		const tool = register(client);
